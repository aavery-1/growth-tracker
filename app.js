...
function planCard(m) {
  ...
  const scol = SM(es).color;
  const tags = (m.tags || []).slice(0, 2).map(t => `<span class="ptag">${esc(t)}</span>`).join('');
  const urgent = t === 'overdue' || t === 'this_month';
  return `<div class="kcard ${urgent ? 'urgent' : ''}" draggable="true" data-id="${m.id}" style="border-left-color:${pr.color}">
    <div class="kc-top"><span class="kc-title" data-expand="${m.id}">${esc(m.activity)}</span></div>
    <div class="kc-tags"><span class="ptag prio prio-${m.priority}" title="Priority"><i style="background:${pr.color}"></i>${pr.label}</span>${tags}</div>
...
function planListHtml() {
  ...
  return keys.map(k => { const ck = 'pg:' + k, open = !state.expanded[ck]; return `<div class="plan-group"><div class="plan-group-head" data-toggle="${esc(ck)}">${chev(open)}${esc(k)}<span class="pm-gcount">${g[k].length}</span></div><div class="plan-cards ${open ? '' : 'hide'}">${g[k].map(planCard).join('')}</div></div>`; }).join('');
}
...
function ragReady(list) {
  if (!list.length) return { key: 'none', color: RAG.none, label: 'No tasks yet' };
  const eff = list.map(effectiveStatus), done = eff.filter(s => s === 'complete').length, pctc = Math.round(100 * done / list.length);
  if (done === list.length) return { key: 'green', color: RAG.green, label: 'All complete' };
  if (eff.some(s => s === 'behind' || s === 'blocked')) return { key: 'red', color: RAG.red, label: 'Behind / Blocked · ' + pctc + '% done' };
  if (eff.some(s => s === 'at_risk')) return { key: 'yellow', color: RAG.yellow, label: 'At risk · ' + pctc + '% done' };
  if (eff.some(s => s === 'on_track')) return { key: 'green', color: RAG.green, label: 'On track · ' + pctc + '% done' };
...
function dashboardHtml(list) {
  ...
  const readiness = `<section class="ex-card"><div class="ex-card-head"><h3>Readiness Index</h3><span class="muted ex-hint">Schedule health by workstream · click a row to open</span></div>
    <div class="ex-grid-wrap"><table class="ex-grid"><thead><tr><th>School</th><th>Opens</th><th>Overall</th>${tms.map(t => `<th class="ex-th-team"><span>${esc(t)}</span></th>`).join('')}</tr></thead><tbody>${grid}</tbody></table></div>
    <div class="ex-legend"><span><i class="rag" style="background:${RAG.green}"></i>On track</span><span><i class="rag" style="background:${RAG.yellow}"></i>At risk</span><span><i class="rag" style="background:${RAG.red}"></i>Behind / Blocked</span><span><i class="rag" style="background:${RAG.none}"></i>Not started</span></div></section>`;
  ...
  const prCard = `<section class="ex-card"><div class="ex-card-head"><h3>Priorities &amp; Risks</h3></div>
    <div class="dash-block-h">Next 90 days<span class="dash-count">${upcoming.length}</span></div>
    ...
    <div class="ex-risks ${risksOpen ? '' : 'hide'}">${stuck.length ? `<div class="ex-list">${stuck.slice(0, 20).map(m => exLi(m, m.status === 'blocked' ? '⛔ ' : '')).join('')}${stuck.length > 20 ? `<div class="muted ex-empty">+ ${stuck.length - 20} more</div>` : ''}</div>` : '<div class="muted ex-empty">Nothing blocked or overdue.</div>'}</div></section>`;

  // GROWTH FUNDRAISING
  const camps = (state.data.campaigns || []).filter(c => !state.filters.states.size || state.filters.states.has(c.state));
  const capital = camps.length ? `<section class="ex-card"><div class="ex-card-head"><h3>Growth Fundraising</h3></div><div class="ex-caps">${camps.map(c => {
    ...
    return `<div class="ex-cap"><div class="ex-cap-top"><b>${esc(c.name)}</b><span>${fmtMoney(c.raised)} <span class="muted">/ ${fmtMoney(c.target)}</span></span></div>
      <div class="ex-cap-bar"><span style="width:${p}%"></span></div><div class="ex-cap-foot muted">${p}% raised · ${esc(c.owner || '')}</div></div>`;
  }).join('')}</div></section>` : '';

  // WORKLOAD — pacing across fiscal years (kept; the donut was redundant, removed)
  const workload = `<section class="ex-card"><div class="ex-card-head"><h3>Workload by Fiscal Year</h3><span class="muted ex-hint">when milestones are due</span></div>${columnChart(list)}</section>`;
...
const SB_DEFAULT = { url: 'https://cwjmlunqfhaioyuijhkk.supabase.co', key: 'sb_publishable_LrqAQcM7hwFlKfdy1dc1Bg_ucLMiggk' };
...
function bootApp() {
  ...
  const sc = sbSavedCfg() || (SB_DEFAULT.url ? SB_DEFAULT : null); if (sc && sc.url && sc.key) sbConnect(sc.url, sc.key, true);
}
